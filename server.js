...
const withTimeout = (promise, ms, label = 'ツール') =>
  Promise.race([promise, new Promise((_, rej) => setTimeout(() => rej(new Error(`${label}がタイムアウトしました（${ms/1000}秒）`)), ms))]);
...
const MAGIC_LINK_DOMAIN = 'acrovision.jp';
...
function requireAuth(req, res, next) {
  ...
  if (req.path === '/auth/magic-link/verify') return next();
  if (req.path === '/auth/magic-link/request') return next();
...
app.post('/auth/magic-link/request', express.json(), async (req, res) => {
  ...
  // 10分以内に3件以上リクエストがある場合はレート制限
  const recent = db.prepare(`SELECT COUNT(*) as c FROM magic_link_tokens WHERE email=? AND created_at > datetime('now','-10 minutes','localtime')`).get(normalizedEmail);
  if (recent.c >= 3) return res.status(429).json({ error: 'しばらく待ってから再試行してください' });

  const token = require('crypto').randomBytes(32).toString('hex');
  db.prepare(`INSERT INTO magic_link_tokens (email, token, expires_at) VALUES (?, ?, datetime('now','+30 minutes','localtime'))`).run(normalizedEmail, token);
  ...
  try {
    await getSesTransport().sendMail({
      ...
      to: normalizedEmail,
      subject: '【AIエージェント】ログインリンク',
      text: `以下のリンクをクリックしてログインしてください（30分間有効）:\n\n${link}\n\nこのメールに心当たりがない場合は無視してください。`,
      html: `<div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:32px;background:#f9fafb;border-radius:12px;">
<h2 style="color:#1a1d2e;margin-bottom:8px;">AIエージェント ログインリンク</h2>
<p style="color:#4a5568;margin-bottom:24px;">下のボタンをクリックしてログインしてください。リンクは<strong>30分間</strong>有効です。</p>
<a href="${link}" style="display:inline-block;padding:14px 28px;background:#5b6eff;color:#fff;border-radius:10px;text-decoration:none;font-weight:700;font-size:15px;">ログインする</a>
<p style="color:#a0aec0;font-size:12px;margin-top:24px;">このメールに心当たりがない場合は無視してください。</p>
</div>`
    });
    res.json({ ok: true });
  } catch (e) {
    console.error('[magic-link] sendMail error:', e.message);
    res.status(500).json({ error: 'メール送信に失敗しました。しばらく後で再試行してください。' });
  }
});

app.get('/auth/magic-link/verify', (req, res) => {
  const { token } = req.query;
  if (!token) return res.redirect('/login?error=magic');
  const row = db.prepare(`SELECT * FROM magic_link_tokens WHERE token=? AND used=0 AND expires_at > datetime('now','localtime')`).get(token);
  if (!row) return res.redirect('/login?error=magic');
  db.prepare('UPDATE magic_link_tokens SET used=1 WHERE id=?').run(row.id);
  const email = row.email;
  const role = getUserRole(email);
  const name = email.split('@')[0];
  const user = { email, name, picture: null, role };
  req.session.regenerate(err => {
    if (err) return res.redirect('/login?error=1');
    req.login(user, loginErr => {
      if (loginErr) return res.redirect('/login?error=1');
      audit(email, name, 'login.magic_link');
      res.redirect('/');
    });
  });
});
...
## ブラウザ操作ツール（管理者専用）
サーバー上のヘッドレスChromiumを操作できる。スクリーンショットで現在の画面を確認しながら操作する:
- \`browser_navigate(url)\` — URL を開く（**最初はこれを使うこと**）
- \`browser_screenshot()\` — 現在の画面を取得
- \`browser_click(selector)\` — CSS セレクターまたはテキストで要素をクリック
- \`browser_type(selector, text)\` — 入力フォームにテキストを入力（submit:true で Enter）
- \`browser_get_text([selector])\` — ページのテキスト内容を取得
...
const TOOLS = [
  ...
  },
  {
    ...
    description: 'サーバー上のヘッドレスブラウザ（Chromium）で指定 URL を開く。スクリーンショットを返す。管理者のみ使用可能。',
    input_schema: {
      type: 'object',
      properties: {
        url: { type: 'string', description: '開く URL（例: https://yahoo.co.jp）' }
      },
      required: ['url']
    }
  },
  {
    ...
    description: '現在のヘッドレスブラウザ画面のスクリーンショットを取得する。管理者のみ使用可能。',
    input_schema: { type: 'object', properties: {} }
  },
  {
    name: 'browser_click',
    description: 'ヘッドレスブラウザ上の要素をクリックする。CSSセレクターかリンクテキストで指定。クリック後のスクリーンショットを返す。管理者のみ使用可能。',
    input_schema: {
      type: 'object',
      properties: {
        selector: { type: 'string', description: 'CSSセレクター（例: #submit, .btn）またはテキスト内容（例: "ログイン"）' }
      },
      required: ['selector']
    }
  },
  {
    name: 'browser_type',
    description: 'ヘッドレスブラウザ上の入力フィールドにテキストを入力する。入力後のスクリーンショットを返す。管理者のみ使用可能。',
    input_schema: {
      type: 'object',
      properties: {
        selector: { type: 'string', description: 'CSSセレクター（例: input[name="q"], #email）' },
        text:     { type: 'string', description: '入力するテキスト' },
        submit:   { type: 'boolean', description: 'true にすると入力後 Enter を押す（デフォルト: false）' }
      },
  ...
  },
  {
    ...
    description: 'ヘッドレスブラウザの現在ページのテキスト内容を取得する。管理者のみ使用可能。',
    input_schema: {
      type: 'object',
      properties: {
        selector: { type: 'string', description: '対象要素の CSS セレクター。省略時はページ全体' }
      }
    }
  }
];
...
async function getCwPage(email) {
  ...
  if (!entry || entry.page.isClosed()) {
    if (entry?.ctx) { entry.ctx.close().catch(() => {}); }
...
下記JSON形式で回答してください（説明文不要）:
{
  ...
  "criteria_scores": {"評価軸名": スコア0〜100}
    ...
    case 'browser_navigate': {
      ...
      const result = await cwScreenshot(page);
      result.title = title;
      result.url = page.url();
      return result;
    }
    ...
    case 'browser_screenshot': {
      if (user.role !== 'admin') throw new Error('browser_screenshot は管理者のみ使用できます');
      const page = await getCwPage(user.email);
      const result = await cwScreenshot(page);
      result.url = page.url();
    ...
    case 'browser_click': {
      if (user.role !== 'admin') throw new Error('browser_click は管理者のみ使用できます');
      const page = await getCwPage(user.email);
      try {
    ...
    case 'browser_type': {
      if (user.role !== 'admin') throw new Error('browser_type は管理者のみ使用できます');
      const page = await getCwPage(user.email);
    ...
    case 'browser_get_text': {
      if (user.role !== 'admin') throw new Error('browser_get_text は管理者のみ使用できます');
      const page = await getCwPage(user.email);
      const text = input.selector
        ? await page.locator(input.selector).first().innerText({ timeout: 5000 }).catch(() => '')
        : await page.evaluate(() => document.body.innerText);
      return { text: text.slice(0, 10000), url: page.url() };
    }
  ...
  try {
    ...
    while (toolRound < 10) {
      ...
      const toolResults = await Promise.all(toolBlocks.map(async block => {
        try {
          const result = await withTimeout(executeTool(block.name, block.input, user), 60000, block.name);
          let toolContent;
...
async function fetchAnthropicSkills() {
  try {
    const ghTokenRow = db.prepare('SELECT github_token FROM user_settings WHERE github_token IS NOT NULL LIMIT 1').get();
    const ghPat = process.env.GITHUB_PAT || ghTokenRow?.github_token;
    const headers = { 'User-Agent': 'claude-agent-web', Accept: 'application/vnd.github+json', ...(ghPat ? { Authorization: `Bearer ${ghPat}` } : {}) };
...
async function runWorkflow(wf, ownerEmail) {
  let steps;
  try { steps = JSON.parse(wf.steps || '[]'); } catch { return { ok: false, error: 'ワークフロー設定が壊れています（steps が不正な JSON）' }; }
